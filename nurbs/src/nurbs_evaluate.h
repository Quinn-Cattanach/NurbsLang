...
float nurbs<dimension>::basis(size_t dim, size_t index, float x,
                              size_t deg) const {
  if (deg == 0)
  ...
  if (denom2 != 0)
    rhs = (knot[dim][index + deg + 1] - x) / denom2 *
          basis(dim, index + 1, x, deg - 1);

  return lhs + rhs;
}

template <size_t dimension>
int nurbs<dimension>::evaluate(const std::array<float, dimension> &param,
  ...
  *result = vec3f_wgsl();
  ...
  while (!done) {
    ...
    *result = *result + coeff * control[idx];
    denominator += coeff;
