...
template <size_t dimension> struct nurbs {
  ...
  int evaluate(const std::array<float, dimension> &param,
               vec3f_wgsl *result) const;

  int bend(size_t param_dim, float param_value, float angle, float radius);

  nurbs<dimension + 1> sweep(const nurbs<1> &path) const;

  std::optional<mesh> tessellate(size_t lod_u, size_t lod_v, size_t index_u,
                                 size_t index_v,
                                 std::array<float, dimension> defaults) const;

  mesh *to_mesh(const std::array<size_t, dimension> &lods) const;

  void to_stl(const std::array<size_t, dimension> &lods,
