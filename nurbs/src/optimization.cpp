...
optimization_result
optimize_grid_search(const std::vector<operation> &construction,
                     const std::vector<bc::n_boundary_condition> &bcs,
                     const material &mat, objective_fn_t objective,
  ...
  for (int combo = 0; combo < total_combinations; ++combo) {
    ...
    for (const auto &var_name : var_names) {
      ...
      float value = var.min_feasible +
                    (sample_idx / float(n_samples_per_var - 1)) * range;
    ...
    // Evaluate objective
    float obj_value =
        evaluate_objective(construction, bcs, mat, objective, var_values);
...
optimize(const std::vector<operation> &construction,
  ...
  if (n_vars <= 2) {
    ...
    return optimize_grid_search(construction, bcs, mat, objective, variables,
                                10);
  }
