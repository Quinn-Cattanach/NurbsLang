...
gsMultiPatch<real_t> nurbs_to_gismo(const nurbs<3>& geom) {
    ...
    // Create control point matrix (3 rows for x,y,z; columns for each control point)
        ...
        if (bc.type == bc::n_boundary_condition::DIRICHLET) {
            ...
            for (int component = 0; component < 3; ++component) {
                ...
                if (std::fabs(force_component) > 1e-12f) {
                    auto force_func = std::make_shared<gsConstantFunction<real_t>>(force_component, 3);
                    bc_gismo.addCondition(0, side, condition_type::neumann, force_func, component);
                }
            }
        }
    }
...
std::vector<float> calculate_von_mises_stress(
    gsElasticityAssembler<real_t> &assembler,
    ...
    // create a gsCauchyStressFunction (computes Cauchy stress tensor from displacement field)
    gsCauchyStressFunction<real_t> stressFunc(disp_multi, mat.youngs_modulus, mat.poissons_ratio);

    // choose evaluation points in parameter domain (we evaluate per patch—here only patch 0)
    int n_samples = 10;
    ...
    // Evaluate stress function: Expectation is to get a matrix of size (6 x N) for symmetric stress components:
    // order: (sigma_xx, sigma_yy, sigma_zz, sigma_xy, sigma_yz, sigma_zx)
    ...
    for (int c = 0; c < total_samples; ++c) {
        ...
        // von Mises formula:
        // sigma_v = sqrt( 0.5 * ((sxx - syy)^2 + (syy - szz)^2 + (szz - sxx)^2) + 3*(sxy^2 + syz^2 + szx^2) )
        double part1 = 0.5 * ( (sxx - syy)*(sxx - syy) + (syy - szz)*(syy - szz) + (szz - sxx)*(szz - sxx) );
...
float find_max_stress(const std::vector<float>& stress_field) {
    if (stress_field.empty()) return 0.0f;
...
float find_max_displacement(const std::vector<vec3f_wgsl>& displacement_field) {
    if (displacement_field.empty()) return 0.0f;
...
fea_result run_fea(const nurbs<3>& geometry, const std::vector<bc::n_boundary_condition>& bcs, const material& mat) {
    ...
    try {
        ...
        result.stress_field = calculate_von_mises_stress(assembler, displacement, mat);        
        // Find maximum values
